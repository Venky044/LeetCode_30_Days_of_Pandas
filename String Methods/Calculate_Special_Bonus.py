...
def calculate_special_bonus(employees: pd.DataFrame) -> pd.DataFrame:
    def fun(id,name,salary):
        bonus = 0
        if id % 2 != 0 and name[0].upper() != 'M':
            bonus = salary
        return bonus

    employees['bonus'] = np.vectorize(fun)(employees['employee_id'], employees['name'], employees['salary'])
    emp =  employees[['employee_id', 'bonus']]
    return emp.sort_values('employee_id', ascending = True)
