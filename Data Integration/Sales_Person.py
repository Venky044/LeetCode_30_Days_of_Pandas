...
def sales_person(sales_person: pd.DataFrame, company: pd.DataFrame, orders: pd.DataFrame) -> pd.DataFrame:
    ...
    df = pd.merge(company[company['name']=='RED'][['com_id', 'name']], orders[['com_id','sales_id']], how='inner', on='com_id')
    df2 = sales_person[~sales_person['sales_id'].isin(df['sales_id'])]
    return df2[['name']]
...
def sales_person(sales_person: pd.DataFrame, company: pd.DataFrame, orders: pd.DataFrame) -> pd.DataFrame:
    ...
    red_sales_id = orders[orders['com_id'].isin(red_id)]['sales_id']
